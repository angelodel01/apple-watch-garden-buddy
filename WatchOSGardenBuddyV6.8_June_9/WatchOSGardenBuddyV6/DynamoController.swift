...
func getUsersTableEntry(key : String){
    // 1) Configure the query
    let queryExpression = AWSDynamoDBQueryExpression()
     queryExpression.keyConditionExpression = "#uid = :uid"
    queryExpression.expressionAttributeNames = [
    ...
    dynamoDbObjectMapper.query(users.self, expression: queryExpression) { (output: AWSDynamoDBPaginatedOutput?, error: Error?) in
        if error != nil {
            print("The request failed. Error: \(String(describing: error))")
        }
        if output != nil {
            for user in output!.items {
                let curr = user as? users
                let name = curr!._fullname!
                let zones = curr!._zones
                print("response : name = \(name), zones = \(zones)")
                userData.user = userData()
                userData.user?.processResp(resp: zones)
            }
        }
    }
}
